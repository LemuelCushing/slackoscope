...
suite("Settings Behavioral Tests", () => {
  ...
  suite("Inline Settings", () => {
    ...
    test("inline.showTime setting controls timestamp display", async () => {
      ...
      assert.strictEqual(settingsManager.inline.showTime, false, "Should disable time display")

      await config.update("inline.showTime", true, vscode.ConfigurationTarget.Global)
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.inline.showTime, true, "Should enable time display")
    })
    ...
    test("inline.useRelativeTime setting toggles between relative and absolute time", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.inline.useRelativeTime, true, "Should use relative time")

      await config.update("inline.useRelativeTime", false, vscode.ConfigurationTarget.Global)
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.inline.useRelativeTime, false, "Should use absolute time")
    })
    ...
    test("inline.showUser setting controls user name display", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.inline.showUser, true, "Should show user names")

      await config.update("inline.showUser", false, vscode.ConfigurationTarget.Global)
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.inline.showUser, false, "Should hide user names")
    })
    ...
    test("inline.showChannelName setting controls channel name display", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.inline.showChannelName, true, "Should show channel names")

      await config.update("inline.showChannelName", false, vscode.ConfigurationTarget.Global)
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.inline.showChannelName, false, "Should hide channel names")
    })
    ...
    test("inline.fontSize setting accepts valid font sizes", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.inline.fontSize, 14, "Should accept valid font size")

      await config.update("inline.fontSize", 18, vscode.ConfigurationTarget.Global)
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.inline.fontSize, 18, "Should update font size")
    })
    ...
    test("inline.color setting accepts color values", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.inline.color, "rgba(255, 0, 0, 0.8)", "Should accept color value")

      await config.update("inline.color", "#00ff00", vscode.ConfigurationTarget.Global)
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.inline.color, "#00ff00", "Should update color")
    })
    ...
    test("inline.fontStyle setting toggles between normal and italic", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.inline.fontStyle, "italic", "Should set italic style")

      await config.update("inline.fontStyle", "normal", vscode.ConfigurationTarget.Global)
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.inline.fontStyle, "normal", "Should set normal style")
    })
  })
  ...
  suite("Hover Settings", () => {
    test("hover.showChannel setting controls channel display in hover", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.hover.showChannel, false, "Should hide channel in hover")

      await config.update("hover.showChannel", true, vscode.ConfigurationTarget.Global)
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.hover.showChannel, true, "Should show channel in hover")
    })
    ...
    test("hover.showFiles setting controls file attachment display", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.hover.showFiles, false, "Should hide files")

      await config.update("hover.showFiles", true, vscode.ConfigurationTarget.Global)
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.hover.showFiles, true, "Should show files")
    })
    ...
    test("hover.showFileInfo setting controls file size/type display", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.hover.showFileInfo, false, "Should hide file info")

      await config.update("hover.showFileInfo", true, vscode.ConfigurationTarget.Global)
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.hover.showFileInfo, true, "Should show file info")
    })
  })
  ...
  suite("Highlighting Settings", () => {
    test("highlighting.enabled setting controls URL highlighting", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.highlighting.enabled, false, "Should disable highlighting")

      await config.update("highlighting.enabled", true, vscode.ConfigurationTarget.Global)
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.highlighting.enabled, true, "Should enable highlighting")
    })
    ...
    test("highlighting.todayColor setting accepts color values", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.highlighting.todayColor, "#00ff00", "Should set today color")

      await config.update("highlighting.todayColor", "rgba(0, 255, 0, 0.5)", vscode.ConfigurationTarget.Global)
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.highlighting.todayColor, "rgba(0, 255, 0, 0.5)", "Should update today color")
    })
    ...
    test("highlighting.oldDays setting accepts number of days", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.highlighting.oldDays, 14, "Should set old days threshold")

      await config.update("highlighting.oldDays", 30, vscode.ConfigurationTarget.Global)
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.highlighting.oldDays, 30, "Should update old days threshold")
    })
    ...
    test("highlighting.oldColor setting accepts color values", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.highlighting.oldColor, "#ff0000", "Should set old color")

      await config.update("highlighting.oldColor", "rgba(255, 0, 0, 0.3)", vscode.ConfigurationTarget.Global)
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.highlighting.oldColor, "rgba(255, 0, 0, 0.3)", "Should update old color")
    })
  })
  ...
  suite("Token Settings", () => {
    test("slackoscope.token setting stores Slack API token", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.slackToken, "xoxb-test-token-123", "Should store token")
    })
    ...
    test("slackoscope.linearToken setting stores Linear API token", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.linearToken, "lin_api_test123", "Should store Linear token")
    })
    ...
    test("token settings support 1Password secret references", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.slackToken, "op://vault/slack/token", "Should store 1Password reference")
    })
  })
  ...
  suite("Settings Integration", () => {
    test("multiple settings can be changed simultaneously", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 100))
      settingsManager.refresh()
      assert.strictEqual(settingsManager.inline.showTime, false)
      assert.strictEqual(settingsManager.inline.showUser, true)
      assert.strictEqual(settingsManager.hover.showFiles, false)
    })
    ...
    test("invalid settings values are rejected or coerced", async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 50))
      const fontSize = config.get("inline.fontSize")
